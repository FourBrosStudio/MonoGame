...
namespace Microsoft.Xna.Framework.Graphics
{
	internal class SpriteBatchItem
	{
        ...
        public void Set(ref Matrix transform, Rectangle sourceRect, Vector2 origin, Color color, Vector2 texCoordTL, Vector2 texCoordBR)
        {
            vertexTL.Position = Vector3.Transform(new Vector3((float)-origin.X, (float)-origin.Y, 0f), transform);
            vertexTL.Position.Z = Depth;
            vertexTL.Color = color;
            vertexTL.TextureCoordinate.X = texCoordTL.X;
            vertexTL.TextureCoordinate.Y = texCoordTL.Y;

            vertexTR.Position = Vector3.Transform(new Vector3((float)sourceRect.Width - origin.X, (float)-origin.Y, 0f), transform);
            vertexTR.Position.Z = Depth;
            vertexTR.Color = color;
            vertexTR.TextureCoordinate.X = texCoordBR.X;
            vertexTR.TextureCoordinate.Y = texCoordTL.Y;

            vertexBL.Position = Vector3.Transform(new Vector3((float)-origin.X, (float)sourceRect.Height - origin.Y, 0f), transform);
            vertexBL.Position.Z = Depth;
            vertexBL.Color = color;
            vertexBL.TextureCoordinate.X = texCoordTL.X;
            vertexBL.TextureCoordinate.Y = texCoordBR.Y;

            vertexBR.Position = Vector3.Transform(new Vector3((float)sourceRect.Width - origin.X, (float)sourceRect.Height - origin.Y, 0f), transform);
            vertexBR.Position.Z = Depth;
            vertexBR.Color = color;
            vertexBR.TextureCoordinate.X = texCoordBR.X;
            vertexBR.TextureCoordinate.Y = texCoordBR.Y;
